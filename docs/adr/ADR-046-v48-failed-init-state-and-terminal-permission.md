...
V48 is a DEMO-feed observer using the frozen `v46_hl10_thr0p05_breadth4` internal virtual USD40 paper book. REAL-MONEY LIVE TRADING and broker-order execution remain forbidden.
...
2. Keep the MQL `OnInit` gates authoritative: DEMO account required, `TERMINAL_TRADE_ALLOWED=0`, `TERMINAL_DLLS_ALLOWED=0`.
...
8. A non-empty run id always wins over recovery logic and blocks a second session.
...
- The next Windows run is the empirical gate for whether `Enabled=0` both preserves EA execution and yields `TERMINAL_TRADE_ALLOWED=0`. If it does not, the launcher must fail closed and return launch-scoped diagnostics; no relaxation of the terminal trading-permission gate is authorized.

## Safety

No `OrderSend`, `OrderSendAsync`, `CTrade`, `trade.Buy`, `trade.Sell`, DLL import, or external broker-order path is introduced.
