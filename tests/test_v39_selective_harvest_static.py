...
def test_v39_runner_verifies_evidence_and_one_zip_manifest():
    ...
    assert '"$PY" "$TEST"' in runner
    assert "dependency-free V39 static gate" in runner
...
def test_v39_bootstrap_uses_recovered_branch_without_local_strategy_patch():
    ...
    assert 'checkout -B "$BRANCH" "$REMOTE_REF"' in text
    assert 'reset --hard "$REMOTE_REF"' in text
    assert "git clean" in text
    assert "python - " not in text
    assert "terminal64.exe" not in text.lower()
    assert "metaeditor64.exe" not in text.lower()

def test_secret_scan_targets_tracked_source_not_runtime_dependencies():
    text = read("scripts/secret_scan.py")
    assert "git" in text and "ls-files" in text
    assert ".venv" in text
    assert "site-packages" in text
    assert "OUTPUT_" in text
    assert "working-tree copy" in text

def test_v36_runner_declares_complete_ml_dependency_gate():
    text = read("runtime/v36_sequence_exit/RUN_V36_SEQUENCE_EXIT_DL_GIT_BASH.sh")
    assert "import numpy, pandas, torch, sklearn, scipy" in text
    assert "from sklearn.metrics import roc_auc_score" in text
    assert '"scikit-learn==1.8.0"' in text
    assert "V36 sklearn" in text
    assert "V36 scipy" in text
    assert "V36 training returned without required summary/predictions" in text
...
def _run_without_pytest():
    tests = [
        ...
        test_secret_scan_targets_tracked_source_not_runtime_dependencies,
        test_v36_runner_declares_complete_ml_dependency_gate,
