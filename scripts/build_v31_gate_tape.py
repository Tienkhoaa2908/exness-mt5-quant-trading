...
REFERENCE_SHA = '44c11a98b75c7764e7a07eff245e1864d9dc85acc4a116a5cd162acb241539fc'
CANDIDATES = [
...
def build(common: pathlib.Path, output: pathlib.Path, metadata: pathlib.Path):
    ...
    use=BAR_NUM+EXPERT
    j=pd.merge_asof(t.sort_values('entry_dt'),b[['avail']+use].sort_values('avail'),left_on='entry_dt',right_on='avail',direction='backward')
    ...
    for tm in tests:
        ...
        for name,m in models().items():
            ...
            out=np.empty((len(idx),len(CANDIDATES),2),dtype=np.float32)
            for ci,c in enumerate(CANDIDATES):
                for di,dval in enumerate((1,-1)):
                    xx=B.loc[idx].copy(); xx['direction_num']=dval
    ...
    if h!=REFERENCE_SHA:
        print('NOTE: tape bytes differ from Linux reference; retain generated hash in evidence. Causal protocol and fixed seeds remain mandatory.',file=sys.stderr)
