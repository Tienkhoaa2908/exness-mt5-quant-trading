...
REFERENCE_SHA='0df85b572f8273f6fef8624bbc12cbded1f77bded046c938eaa9ff5e2e7a3f7f'
...
def load_lake(common):
 ...
 b=pd.concat([b1[(b1.dt>=pd.Timestamp('2025-02-01'))&(b1.dt<pd.Timestamp('2025-08-01'))],b2[(b2.dt>=pd.Timestamp('2025-08-01'))&(b2.dt<pd.Timestamp('2026-02-01'))],b3[(b3.dt>=pd.Timestamp('2026-02-01'))&(b3.dt<pd.Timestamp('2026-08-01'))]],ignore_index=True).sort_values('dt').drop_duplicates('dt').reset_index(drop=True)
...
def engineer_bar(df):
 for e in ['ema','macd','bos','trend','slow']:
  df[f'fastslow_{e}']=df[f'ewma_fast5_{e}']-df[f'ewma_slow20_{e}']; df[f'hl8slow_{e}']=df[f'ewma_hl8_{e}']-df[f'ewma_slow20_{e}']; df[f'conf_{e}']=np.log1p(df[f'expert_obs_{e}'])
 df['vol_ratio']=df.rv8/(df.rv32.abs()+1e-9); df['di_spread']=df.plus_di-df.minus_di; df['signal_imbalance']=df.signal_count_long-df.signal_count_short
...
def models():
 return {'catboost':CatBoostRegressor(iterations=80,depth=5,learning_rate=.035,l2_leaf_reg=5,verbose=False,random_seed=3,thread_count=4),'extratrees':ExtraTreesRegressor(n_estimators=80,min_samples_leaf=12,max_features=.65,n_jobs=-1,random_state=3),'mlp':Pipeline([('sc',StandardScaler()),('m',MLPRegressor(hidden_layer_sizes=(64,32,16),alpha=.01,max_iter=80,early_stopping=True,n_iter_no_change=12,random_state=3))]),'linear_svr':Pipeline([('sc',StandardScaler()),('m',LinearSVR(C=.03,epsilon=.05,max_iter=4000,random_state=3))])}
def fit(m,x,y,w):
 try:
...
def build(common,output,metadata):
 ...
 engineer_bar(b)
 months=sorted(j.month.unique()); tests=months[6:]; maskL=np.zeros((len(b),len(CANDIDATES)),dtype=np.uint8); maskS=np.zeros_like(maskL); meta=[]
 avail_table=b[['avail']+basecols].sort_values('avail').reset_index(drop=True)
 for tm in tests:
  cal=months[months.index(tm)-1]; cstart=pd.Timestamp(cal.replace('_','-')+'-01'); train=j.exit_dt<cstart; calmask=j.month==cal; bm=b.month==tm; idx=np.where(bm)[0]
  ...
  for name,m in models().items():
   fit(m,X.loc[train],y[train],w[train]); thrs[name]=float(np.quantile(m.predict(X.loc[calmask]),.50)); out=np.empty((len(idx),len(CANDIDATES),2),dtype=np.float32)
  ...
  print(f'gate month PASS test={tm} cal={cal} train={int(train.sum())} bars={len(idx)}',flush=True)
 oos=b.month.isin(tests); idx=np.where(oos)[0]; output.parent.mkdir(parents=True,exist_ok=True); metadata.parent.mkdir(parents=True,exist_ok=True)
 with output.open('w',newline='') as f:
  ...
  for k in idx:
   ...
   f.write(','.join(vals)+'\n')
 payload={'models_bits':{'0':'catboost','1':'extratrees','2':'deep_mlp_64_32_16','3':'linear_svr','4':'catboost_AND_extratrees','5':'majority_2of4'},'candidate_order':CANDIDATES,'months':meta,'feature_count':len(X.columns),'protocol':'fit exits before prior calibration month; threshold=median prior-month scores; inverse opportunity weighting; at each actual MT5 current bar T score latest feature_available_time<=T','versions':{'numpy':np.__version__,'pandas':pd.__version__}}
 metadata.write_text(json.dumps(payload,indent=2),encoding='utf-8'); h=sha256(output); print(f'V31 gate tape PASS rows={len(idx)} sha256={h} reference_sha256={REFERENCE_SHA}')
 if h!=REFERENCE_SHA: print('NOTE: tape bytes differ from Linux reference; retain generated hash in evidence. Causal row-count and current-bar asof contract are mandatory.',file=sys.stderr)
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--common-files',required=True); ap.add_argument('--output',required=True); ap.add_argument('--metadata',required=True); a=ap.parse_args(); build(pathlib.Path(a.common_files),pathlib.Path(a.output),pathlib.Path(a.metadata))
