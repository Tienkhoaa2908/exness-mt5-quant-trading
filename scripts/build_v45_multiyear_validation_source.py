...
NEW_RELEASE = "v45_multiyear_single_run_validation_v1"
...
def build(source: pathlib.Path, output: pathlib.Path) -> None:
    ...
    text = replace_once(text, f'#define MT5Q_RELEASE_ID "{EXPECTED_PARENT_RELEASE}"', f'#define MT5Q_RELEASE_ID "{NEW_RELEASE}"')
    text = replace_once(text, 'input string InpOutputTag = "v38_fast_harvest_lab_v1";', 'input string InpOutputTag = "v45_multiyear_single_run_validation_v1";')
    text = replace_once(text, 'input bool   InpV34WriteIntraTradeTelemetry = true;', 'input bool   InpV34WriteIntraTradeTelemetry = false;')
    text = replace_once(text, 'input bool   InpV38WriteM1FastTelemetry = true;', 'input bool   InpV38WriteM1FastTelemetry = false;')
    ...
    text = text.replace("V38_FAST_HARVEST_LAB START", "V45_MULTIYEAR_VALIDATION START")
    text = text.replace("V38_FAST_HARVEST_LAB DONE", "V45_MULTIYEAR_VALIDATION DONE")
    ...
    for token in [
        ...
        if token not in text:
            raise RuntimeError(f"V45 marker missing after build: {token}")
    for bad in FORBIDDEN:
        if bad in text:
    ...
    print(f"V45 source PASS sha256={sha256(output)} path={output}")
