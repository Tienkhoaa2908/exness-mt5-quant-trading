...
OUT="$ROOT/OUTPUT_V43"
CP="$OUT/checkpoints"
LOG="$OUT/v43_confidence_router_runner.log"
...
V43_SOURCE_SHA="487f2fffdfb7a348bd697fc0a8e6682d39a83f06b1a09453f7a194d5f5000c8a"
...
EXPECTED_BRANCH="agent/v43-confidence-aware-router-exact-mt5"

STATE1="$REPO_ROOT/runtime/v34_parallel_alpha/state_after_chunk1.csv"
F34="$REPO_ROOT/scripts/v34_parallel_alpha_features.py"
S43="$REPO_ROOT/scripts/build_v43_confidence_router_source.py"
A43="$REPO_ROOT/scripts/analyze_v43_confidence_router_mt5.py"
TEST="$REPO_ROOT/tests/test_v43_confidence_router_static.py"
SECRET_SCAN="$REPO_ROOT/scripts/secret_scan.py"
PACKAGER="$REPO_ROOT/scripts/package_research_bundle_portable.py"
BOOT="$ROOT/BOOTSTRAP_V43_CONFIDENCE_ROUTER_ONE_SHOT_GIT_BASH.sh"
...
say "V43 CONFIDENCE-AWARE ROUTER — EXACT MT5"
...
for f in "$STATE1" "$F34" "$S43" "$A43" "$TEST" "$SECRET_SCAN" "$PACKAGER" "$BOOT"; do
...
say "Compile + V43 static tests + tracked-source secret scan"
"$PY" -m py_compile "$F34" "$S43" "$A43" "$TEST" "$SECRET_SCAN" "$PACKAGER"
...
then
  ...
  echo "pytest unavailable: running dependency-free V43 gate"
  "$PY" "$TEST"
fi
...
say "Deterministic source reconstruction: accepted V38 -> V43"
BASE43A="$OUT/V43ConfidenceRouterLab.base.a.mq5"
BASE43B="$OUT/V43ConfidenceRouterLab.base.b.mq5"
"$PY" "$S43" --source "$(cygpath -w "$BASE38A")" --output "$(cygpath -w "$BASE43A")"
"$PY" "$S43" --source "$(cygpath -w "$BASE38A")" --output "$(cygpath -w "$BASE43B")"
SHA43A="$(sha256sum "$BASE43A" | awk '{print $1}')"
SHA43B="$(sha256sum "$BASE43B" | awk '{print $1}')"
[[ "$SHA43A" == "$SHA43B" ]] || die "V43 deterministic double-build mismatch"
[[ "$SHA43A" == "$V43_SOURCE_SHA" ]] || die "V43 source hash mismatch expected=$V43_SOURCE_SHA actual=$SHA43A"
echo "V38_PARENT_ZIP_SHA=$ACTUAL_V38_ZIP_SHA"
echo "V38_PARENT_SOURCE_SHA=$SHA38A"
echo "V43_SHA=$SHA43A"

"$PY" - "$BASE43A" <<'PYMQLLINT'
from pathlib import Path
import re,sys
p=Path(sys.argv[1])
...
for token in [
    ...
    '#define CANDIDATE_COUNT 27',
    'v43_confidence_aware_router=1',
    'v43_risk_changed=0',
    'v43_entry_exit_geometry_changed=0',
    'v43_global_time_hysteresis=0',
    'ResolveV43ConfidenceSignal',
...
if bad:
    print('\n'.join(bad))
    raise SystemExit(78)
print('Generated V43 MQL lint PASS:',p)
PYMQLLINT
...
compile_checkpoint_valid(){
  ...
  local u8="$OUT/.compile_v43_reuse_check.txt"
  local sum=""
...
PYMTIME
  ...
  printf '%s\n' "$expected_sha" > "$marker"
  cp -f "$u8" "$OUT/V43ConfidenceRouterLab.compile.txt"
...
collect_run(){
  ...
  collect_ready "$rd" "$marker" || { ls -la "$rd" || true; die "run folder incomplete after artifact wait: $rd"; }
...
run_mt5_checkpoint(){
  ...
  local backup="$OUT/state_before_v43.csv"
  local had_state=0
  ...
  say "RUN v43_confidence_router — exact MT5 Deposit=40, Model=0, tester-only, risk<=1.00%"
  ...
  [[ -s "$state_target" ]] && cp -f "$state_target" "$OUT/state_after_v43.csv"
...
EA43="$EXPERT_DIR/V43ConfidenceRouterLab.mq5"
if [[ -s "$EA43" && "$(sha256sum "$EA43" | awk '{print $1}')" == "$SHA43A" ]]; then
  say "REUSE installed V43 source bytes sha=$SHA43A"
else
  cp -f "$BASE43A" "$EA43"
fi
say "Compile/reuse V43 Confidence-Aware Router Lab with direct MetaEditor artifact gate"
compile_ea "$EA43" "$SHA43A"

run_mt5_checkpoint "v43_confidence_router" "V43ConfidenceRouterLab" "2025.08.01" "2026.08.01" "$STATE1" "v43_confidence_aware_router=1"

ANJSON="$OUT/v43_confidence_router_analysis.json"
ANCSV="$OUT/v43_confidence_router_comparison.csv"
"$PY" "$A43" --run-folder "$(cygpath -w "$CP/v43_confidence_router")" --output "$(cygpath -w "$ANJSON")" --csv "$(cygpath -w "$ANCSV")"

EVID="$OUT/V43_EVIDENCE.txt"
{
  echo "schema=v43_confidence_aware_router_exact_mt5_evidence_v1"
  echo "head=$HEAD"
  echo "branch=$BRANCH"
  echo "runner_architecture=direct_v32_v34_v38_style_artifact_gated_portable_packaging_v1"
  echo "v30_sha=$V30_SHA"
  echo "v38_parent_zip_sha=$ACTUAL_V38_ZIP_SHA"
  echo "v38_parent_source_sha=$SHA38A"
  echo "v43_source_sha=$SHA43A"
  echo "v43_expected_source_sha=$V43_SOURCE_SHA"
  echo "v43_global_time_hysteresis=0"
...
for f in \
  "$EVID" \
  "$LOG" \
  "$OUT/V43ConfidenceRouterLab.compile.txt" \
  "$BASE38A" \
  "$BASE43A" \
  "$ANJSON" \
  "$ANCSV" \
...
for f in DONE.txt MT5_DONE.txt LATEST.txt SOURCE_RUN_FOLDER.txt manifest.txt monthly_summary.csv trades.csv; do
  p="$CP/v43_confidence_router/$f"
  [[ -s "$p" ]] && cp -f "$p" "$BUNDLE_DIR/$f" || true
done

ZIP="$OUT/v43_confidence_router_exact_mt5.zip"
rm -f "$ZIP"
"$PY" "$PACKAGER" --bundle "$(cygpath -w "$BUNDLE_DIR")" --output "$(cygpath -w "$ZIP")"

ZIP_SHA="$(sha256sum "$ZIP" | awk '{print $1}')"
say "V43 EXACT-MT5 DONE"
"$PY" - "$ANJSON" <<'PYPRINT'
import json,sys
