...
ROOT="$(git rev-parse --show-toplevel 2>/dev/null)" || { echo "FATAL: not inside git repository"; exit 70; }
cd "$ROOT"
...
if [ -n "$(git status --porcelain)" ]; then
  echo "FATAL: working tree must be clean before V61 research"
...
win_to_unix() {
  local p="$1"
  if command -v cygpath >/dev/null 2>&1; then cygpath -u "$p" 2>/dev/null || printf '%s\n' "$p"; else printf '%s\n' "$p"; fi
}
...
if [ -z "$PY" ] || [ ! -x "$PY" ]; then echo "FATAL: no usable Python 3 interpreter found"; exit 73; fi
"$PY" -c 'import sys; assert sys.version_info >= (3,10)' || { echo "FATAL: Python >=3.10 required"; exit 76; }

echo "============================================================"
echo "V61 PROFIT-RATCHET + M5 REFINEMENT RESEARCH"
echo 'fixed 0.01 | risk band $0.75-$1.25 | target $3'
echo 'arm at +$2 -> lock +$1 | M5 structural refinement | OrderCheck'
...
"$PY" runtime/v61_profit_ratchet_m5_refinement/RUN_V61_PROFIT_RATCHET_M5_REFINEMENT.py
