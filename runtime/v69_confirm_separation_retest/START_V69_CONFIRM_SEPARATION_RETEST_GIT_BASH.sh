...
RUNNER="runtime/v69_confirm_separation_retest/RUN_V69_CONFIRM_SEPARATION_RETEST.py"
STATIC="tests/test_v69_confirm_separation_retest_static.py"

printf '%s\n' '============================================================'
printf '%s\n' 'V69 CONFIRM-SEPARATION-RETEST RESEARCH'
printf '%s\n' 'reclaim confirm -> favorable separation -> later cash-zone retest'
printf '%s\n' 'fixed 0.01 | planned risk $0.85-$1.10 | TP $3.50'
printf '%s\n' '18 Model=4 replay passes | LONG/SHORT independent'
printf '%s\n' '============================================================'
...
if tasklist.exe 2>/dev/null | grep -qi 'terminal64.exe'; then
  printf '%s\n' 'FATAL: MT5 is open; close it before V69'
  exit 103
fi
if tasklist.exe 2>/dev/null | grep -qi 'metaeditor64.exe'; then
  printf '%s\n' 'FATAL: MetaEditor is open; close it before V69'
  exit 104
fi
