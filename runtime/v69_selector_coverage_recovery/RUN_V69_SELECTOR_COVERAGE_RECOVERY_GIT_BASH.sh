...
EXPECTED="${V69_SELECTOR_COVERAGE_EXPECTED_HEAD:-}"
if [ -z "$EXPECTED" ]; then
    echo "FATAL: V69_SELECTOR_COVERAGE_EXPECTED_HEAD is required"
    exit 20
fi

echo "[$(date '+%Y-%m-%d %H:%M:%S')] V69 selector coverage recovery"
echo "BRANCH=$(git branch --show-current)"
echo "HEAD=$(git rev-parse HEAD)"
echo "EXPECTED_HEAD=$EXPECTED"

[ "$(git rev-parse HEAD)" = "$EXPECTED" ] || {
    echo "FATAL: HEAD MISMATCH"
    exit 21
}
...
$PYTHON tests/test_v69_selector_coverage_recovery.py
$PYTHON scripts/secret_scan.py .

$PYTHON runtime/v69_selector_coverage_recovery/RUN_V69_SELECTOR_COVERAGE_RECOVERY.py

echo "V69_SELECTOR_COVERAGE_LAUNCHER=PASS"
