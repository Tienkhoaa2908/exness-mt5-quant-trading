...
def build_compile_install(runner, data: Path, expert_dir: Path) -> tuple[Path, Path, str, str, Path]:
    ...
    print(f"V69_FROZEN_SOURCE_SHA256={source_sha}")
    if source_sha != FROZEN_FORWARD_SOURCE_SHA256:
        raise RuntimeError(
            f"frozen forward source drift expected={FROZEN_FORWARD_SOURCE_SHA256} actual={source_sha}"
        )
    ...
    if not installed_source.is_file() or runner.sha(installed_source) != source_sha:
        raise RuntimeError("installed frozen MQ5 identity mismatch")
    if not installed_ex5.is_file() or installed_ex5.stat().st_size <= 0:
        raise RuntimeError("compiled frozen EX5 missing")
    ex5_sha = runner.sha(installed_ex5)

    # Startup Expert= is most reliable at the Experts root. Copy exact bytes only;
    # no source or strategy transformation happens here.
    ...
    if runner.sha(startup_source) != source_sha or runner.sha(startup_ex5) != ex5_sha:
        raise RuntimeError("startup expert byte-copy verification failed")

    print(f"V69_FROZEN_EX5_SHA256={ex5_sha}")
    print("V69_STARTUP_EXPERT_COPY_PASS=1")
    return startup_source, startup_ex5, source_sha, ex5_sha, compile_log
...
def wait_ready(runner, root: Path, proc: subprocess.Popen) -> dict[str, str]:
    status = root / "V64_STATUS.txt"
    deadline = time.time() + 150
    while time.time() < deadline:
        s = parse_kv(status)
        if s:
            state = s.get("state", "")
            if state == "READY" and s.get("symbol") == SYMBOL and s.get("fixed_lot") in {"0.01", "0.010"}:
                print("V69_FORWARD_DEMO_READY=1")
                return s
            if state == "STOPPED":
        ...
        time.sleep(1)
    raise RuntimeError("timeout waiting for V69 fresh READY telemetry")
...
def main() -> int:
    ...
    run([sys.executable, "-m", "py_compile", BUILDER, FORWARD_STATIC, ONE_SHOT_STATIC, SUPERVISOR, Path(__file__)])
    ...
    if runner.base.task_running("terminal64.exe"):
        raise RuntimeError(
            ...
            "the runner will relaunch the exact frozen EA automatically."
        )
    if runner.base.task_running("metaeditor64.exe"):
        raise RuntimeError(
            "MetaEditor is already running. Close MetaEditor once, then rerun this SAME one-shot command."
        )
    ...
    startup_source, startup_ex5, source_sha, ex5_sha, compile_log = build_compile_install(
        runner, Path(data), expert_dir
    )
    root = archive_forward_common(Path(common))
    ini = write_start_config(runner, Path(data))

    launched_at = datetime.now(timezone.utc)
    print("LAUNCH_V69_FROZEN_FORWARD_DEMO=1")
    proc = subprocess.Popen(
    ...
    status = wait_ready(runner, root, proc)

    session = {
        "protocol": "v69_frozen_forward_one_shot_v1",
        ...
        "frozen_forward_source_sha256": source_sha,
        "frozen_forward_ex5_sha256": ex5_sha,
        ...
        "early_review_min_closed_trades": 12,
        "early_review_min_elapsed_days": 14,
    ...
    print("Git Bash may be closed after PASS. Keep PC, Internet and MT5 running.")
    return 0
