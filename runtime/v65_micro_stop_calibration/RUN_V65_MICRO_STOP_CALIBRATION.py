...
EXPECTED_BRANCH = "agent/v65-micro-stop-calibration-research"
SYMBOL = "XAUUSDm"
PERIOD = "M15"
REAL_MODEL = 4
...
# Frozen from accepted V64 PnL-independent screen evidence. V65 does not
# reselect windows, so the only experimental change is entry/stop semantics.
BEARISH_WINDOWS = [
...
DIRECTIONS = (
    ("long", 1, "V65MicroStopCalibrationLong"),
    ("short", -1, "V65MicroStopCalibrationShort"),
)

HERE = Path(__file__).resolve().parent
REPO = HERE.parents[1]
OUT = HERE / "OUTPUT_V65"
ZIP_OUT = OUT / "v65_micro_stop_calibration_research.zip"
BUILDER = REPO / "scripts" / "build_v65_micro_stop_calibration_source.py"
STATIC_TEST = REPO / "tests" / "test_v65_micro_stop_calibration_static.py"
...
V64_ANALYZER = REPO / "scripts" / "analyze_v64_microstructure_trigger_shadow.py"
SECRET_SCAN = REPO / "scripts" / "secret_scan.py"
ADR = REPO / "docs" / "adr" / "ADR-067-v65-micro-stop-calibration-research.md"
HANDOFF = REPO / "docs" / "handoff" / "V65_RECOVERY_STATE.md"
...
def ensure_repo() -> tuple[str, str]:
    ...
    if dirty:
        raise RuntimeError("working tree must be clean before V65 research")
    return branch, head


def configure_v64_runtime():
    runner = load(V64_RUNNER, "v64_runtime_reused_by_v65")
    fixed = load(V64_FIXED, "v64_fixed_helpers_for_v65")
    fixed.install_mt5_locator_compat(runner.base)

    runner.OUT = OUT
    runner.COMMON_DIR = "v65_micro_stop_calibration"
...
def build_sources(runner) -> list[tuple[str, int, str, Path, str]]:
    ...
    for label, direction, expert in DIRECTIONS:
        ...
        print(f"V65_SOURCE_PASS direction={label.upper()} expert={expert} sha256={digest}")
        built.append((label, direction, expert, source, digest))
    return built


def analyze(run_dirs: list[Path]) -> tuple[Path, Path]:
    analysis = OUT / "v65_analysis.json"
    summary = OUT / "V65_SUMMARY.txt"
    cmd = [sys.executable, V64_ANALYZER]
    for rd in run_dirs:
        cmd += ["--run-dir", rd]
    ...
    if summary.is_file():
        ...
        summary.write_text(text.replace("V64_MICROSTRUCTURE_TRIGGER_SHADOW_ANALYSIS", "V65_MICRO_STOP_CALIBRATION_ANALYSIS"), encoding="utf-8")
    return analysis, summary


def package(runner, branch: str, head: str, built, compiles: list[Path], run_dirs: list[Path]) -> None:
    protocol = OUT / "V65_PROTOCOL.json"
    protocol.write_text(json.dumps({
        ...
        "micro_trigger_precedes_cash_risk_gate": True,
        "actual_stop_source": "m1_micro_structural_invalidation",
        "m5_role": "context_structure_only",
        "tester_only": True,
        "real_money_authorized": False,
    }, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    evidence = OUT / "V65_EVIDENCE.txt"
    evidence.write_text("\n".join([
        "V65_MICRO_STOP_CALIBRATION_EVIDENCE=1",
        f"BRANCH={branch}",
        f"HEAD={head}",
        "FIXED_LOT=0.01",
        ...
        "ACTUAL_TARGET=3.50",
        "MIN_RISK_SPREAD_RATIO=4.0",
        "MICRO_TRIGGER_BEFORE_RISK_GATE=1",
        "ACTUAL_STOP_SOURCE=M1_MICRO_STRUCTURAL_INVALIDATION",
        "M5_ROLE=CONTEXT_ONLY",
        "FROZEN_V64_WINDOWS=1",
        "MODEL4_PASSES=12",
        "TESTER_ONLY=1",
        "REAL_MONEY_AUTHORIZED=0",
    ]) + "\n", encoding="utf-8")

    include = [x[3] for x in built] + compiles + [protocol, evidence, OUT / "v65_analysis.json", OUT / "V65_SUMMARY.txt", ADR, HANDOFF]
    for rd in run_dirs:
        if rd.exists():
            include += [p for p in sorted(rd.rglob("*")) if p.is_file()]
    include = [p for p in include if p.is_file()]

    manifest = OUT / "V65_MANIFEST_SHA256.txt"
    rows = []
    for p in include:
        try:
    ...
    with zipfile.ZipFile(ZIP_OUT) as zf:
        bad = zf.testzip()
        if bad is not None:
            raise RuntimeError(f"V65 package CRC failure first_bad={bad}")
    print(f"V65_PACKAGE_PASS=1 files={len(include)}")
    print(f"V65_ZIP={ZIP_OUT}")
    print(f"V65_ZIP_SHA256={runner.sha(ZIP_OUT)}")
...
def main() -> int:
    ...
    print(f"V65_MT5_LOCATOR_COMPAT=PASS data={data} common={common} expert_dir={expert_dir}")
    print("V65_COMPILE_DIAGNOSTICS=ENABLED")
