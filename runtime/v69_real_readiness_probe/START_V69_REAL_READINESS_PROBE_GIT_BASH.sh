...
RUNNER="$ROOT/RUN_V69_REAL_READINESS_PROBE.py"
STATIC="$REPO_ROOT/tests/test_v69_real_readiness_probe_static.py"
EXPECTED_BRANCH="agent/v69-one-shot-prospective-demo"
EXPECTED_HEAD="${V69_REAL_READINESS_EXPECTED_HEAD:-}"
...
[[ -n "$EXPECTED_HEAD" ]] || die "V69_REAL_READINESS_EXPECTED_HEAD is required"
...
bash -n "$0"
"${PY_CMD[@]}" -m py_compile \
  "$REPO_ROOT/scripts/build_v69_demo_execution_probe_source.py" \
  "$REPO_ROOT/scripts/analyze_v69_live_signal_path.py" \
  "$RUNNER" "$STATIC"
"${PY_CMD[@]}" "$STATIC"
"${PY_CMD[@]}" "$REPO_ROOT/scripts/secret_scan.py" "$REPO_ROOT"

say "Start V69 immediate real-readiness DEMO probe"
"${PY_CMD[@]}" "$RUNNER"

echo
echo "V69_REAL_READINESS_PROBE_LAUNCHER=PASS"
