...
EXPECTED="${V69_MFE_GIVEBACK_EXPECTED_HEAD:-}"
if [ -z "$EXPECTED" ]; then
    echo "FATAL: V69_MFE_GIVEBACK_EXPECTED_HEAD is required"
    exit 20
fi

echo "[$(date '+%Y-%m-%d %H:%M:%S')] V69 MFE/giveback recovery"
...
$PYTHON tests/test_v69_mfe_giveback_recovery.py
$PYTHON scripts/secret_scan.py .
$PYTHON runtime/v69_mfe_giveback_recovery/RUN_V69_MFE_GIVEBACK_RECOVERY.py

echo "V69_MFE_GIVEBACK_LAUNCHER=PASS"
